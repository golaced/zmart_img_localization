...
#include <Eigen/Dense>
#include “zmart_img/zmart_img_conversion.h”

using namespace std;
using namespace Eigen;

/***
    Euler angle defination:zyx so the order is yaw ptich roll
    Rotation matrix: C_body2ned
***/

Quaterniond euler2quaternion(Vector3d euler)
{
    double cr = cos(euler(0)/2); //cos(yaw/2)
    double sr = sin(euler(0)/2); //sin(yaw/2)
    double cp = cos(euler(1)/2); //cos(pitch/2)
    double sp = sin(euler(1)/2); //sin(pitch/2)
    double cy = cos(euler(2)/2); //cos(roll/2)
    double sy = sin(euler(2)/2); //sin(roll/2)
